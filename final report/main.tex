...
Political leaders often direct resources toward their home regions, a pattern documented in cross-country evidence using subnational nightlights \cite{HodlerRaschky2014}. This paper asks whether that result still holds in later periods. The post-2005 years are especially useful for that question because they overlap with harmonized nightlights and global pollution measures. They are also exactly where the canonical birth-region activity effect becomes much harder to detect.
...
The contribution relative to recent replication work is therefore diagnostic. Bora (2025) revisits birthplace favoritism in an open-source framework and documents robust modern evidence with VIIRS data. By contrast, we focus on the annual post-2005 windows that overlap with our pollution outcomes, test theory-favored subsamples where the nightlights reduced form should be strongest, and evaluate whether a shift toward cleaner growth can explain the weaker activity effect. The result is not that the historical finding disappears in all data. It is that in the later annual windows we study, the classic birth-region nightlights effect is much weaker and statistically uninformative.
...
The main result is that the canonical birth-region nightlights reduced form becomes much weaker in later periods, especially after 2005. The long DMSP replication still holds, especially in Africa and Sub-Saharan Africa, but the later-period estimates are close to zero and statistically uninformative in the pooled sample and in the most natural diagnostic subsamples. This points to a change over time in the strength of the original finding, not just to a change in how nightlights are measured.
