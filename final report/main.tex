...
\significancestatement{Regional favoritism is a well-known finding in political economy: leaders tend to direct economic activity toward their birth regions. Many recent applications study this mechanism in the post-2005 satellite era, when harmonized nightlights and global pollution data are available. This paper shows that the canonical birth-region nightlights reduced form is much weaker in that later era than in the long DMSP replication window. The attenuation is broad across geographic, political, and measurement-motivated subsamples. Pollution outcomes do not suggest that leaders shifted from directing more growth to directing cleaner growth toward birth regions.}
...
\begin{abstract}
We ask whether the canonical birth-region favoritism result in nightlights persists in the post-2005 satellite era used by much recent research. We combine the Political Leaders' Affiliation Database, ADM2-level nightlights, ACAG surface NO$_2$, ACAG PM$_{2.5}$, V-Dem democracy scores, and GADM administrative boundaries in a global district-year panel with region fixed effects and country-year fixed effects. The long DMSP replication window recovers the classic result: the lag-0 coefficient is 0.0128 in 1992--2013 ($p = 0.030$) and remains positive in nearby historical windows. The post-2005 estimates are much smaller and statistically uninformative: 0.0017 in DMSP 2005--2013 ($p = 0.768$) and 0.0004 in harmonized 2005--2019 ($p = 0.966$). A targeted search across geography, tenure, autocracy, luminosity headroom, and pollution-support subsamples shows that Africa and Sub-Saharan Africa drive the historical result, but no post-2005 subsample restores a precise activity effect. Pollution outcomes test one explanation for attenuation: favoritism may have shifted from the quantity to the composition of growth. The estimates do not support that interpretation. Pooled NO$_2$ and pollution-intensity estimates are positive, though less robust under country clustering, and PM$_{2.5}$ results are also not negative. The main contribution is to document broad post-2005 attenuation and to show that cleaner birth-region growth does not explain it.
\end{abstract}
...
Political leaders often direct resources toward their home regions, a pattern documented in cross-country evidence using subnational nightlights \cite{HodlerRaschky2014}. The question for current empirical work is whether that result remains a strong building block in the satellite era researchers now use most often. Post-2005 data are attractive because they overlap with harmonized DMSP--VIIRS nightlights, VIIRS-era products, and global pollution measures. They are also exactly where the canonical birth-region activity effect becomes much harder to detect.

This paper documents that attenuation and asks what it implies. In the long DMSP window, the lag-0 birth-region coefficient is positive and statistically significant. In 2005--2013, still within DMSP, and in harmonized 2005--2019, the point estimates fall close to zero and become statistically uninformative. The attenuation is not confined to one endpoint choice. It also survives targeted cuts by geography, leader tenure, autocracy, luminosity headroom, and pollution-support samples.
...
The contribution relative to recent replication work is therefore diagnostic. Bora (2025) revisits birthplace favoritism and shows that data choice and periodization matter; we extend that logic to the post-2005 window where pollution data become available, test theory-favored subsamples where the nightlights reduced form should be strongest, and evaluate whether a shift toward cleaner growth can explain the weaker activity effect. The result is not that the historical finding disappears in all data. It is that the post-2005 signal is too weak and broad-based to support a simple continuation of the classic birth-region favoritism design.
...
Standard errors are clustered by leader spell in the baseline, following the leader-term clustering convention in recent replication work such as \cite{Bora2025}. Because \cite{HodlerRaschky2014} use country-level clustering, we also report country-clustered robustness checks for the main estimates. Following the baseline specification in \cite{HodlerRaschky2014}, we estimate lag 0, lag 1, and lag 2 treatment indicators, and focus on lag 0 in the main text as the most direct test of the regional favoritism hypothesis.

Our later-period environmental outcomes are log nightlights, log NO$_2$, and a pollution-intensity measure defined as $\log(\text{NO}_2 + 0.01) - \log(\text{Nightlights} + 0.01)$. Because both component regressions use the same regressor and fixed effects, the intensity coefficient is mechanically a linear combination of the NO$_2$ and nightlights coefficients ($\beta_{intensity} = \beta_{NO2} - \beta_{lights}$). We report it as a compact summary of whether pollution rises more or less than lights in birth regions, not as an independent source of identifying variation. In the PM$_{2.5}$ extension, the analogous outcome is $\log(\text{PM}_{2.5} + 0.01) - \log(\text{Nightlights} + 0.01)$.
...
The heterogeneity checks do not recover the later activity effect either. Restricting the sample to country-years with $v2x\_polyarchy < 0.3$ yields a lag-0 nightlights coefficient of 0.0111 ($p = 0.574$) in the DMSP 1992--2013 sample and 0.0076 ($p = 0.552$) in the DMSP 2005--2013 sample (Table~\ref{tab:autocracy}). These estimates are positive but too imprecise to rescue the historical result.
...
The main result is that the canonical birth-region nightlights reduced form becomes much weaker in the post-2005 satellite era. The long DMSP replication still holds, especially in Africa and Sub-Saharan Africa, but the later-period estimates are close to zero and statistically uninformative in the pooled sample and in the most natural diagnostic subsamples. This matters because many current applications use post-2005 satellite products and leader-birthplace designs as if the historical activity effect carries forward unchanged.
